...
| 27 | decision | proposed | **Step 4 is at the merge gate.** Build stopped before merging, as `## Authorized now` instructs. Review `docs/reviews/0012-step-4-wiring-availability.md` and the diff, then decide the merge. Three things in that report want a decision rather than a nod: (a) check (2) is met more narrowly than its wording — `not_applicable` is unreachable live, so the registry half stops lying by saying "we read it and it did not carry this", never "they never file this"; (b) a section now leaves the offer when all of its *screening-required* facts are settled, which also stops chasing four financial facts screening does not require; (c) `registry.retrieved` is only as good as `filing_fetched`, whose host list (`classifySourceType`, `research-actions.ts:74-88`) will miss a filing read from anywhere else — it errs toward offering more work, and the rate is unmeasured. | 2026-09-17 |
...
| 24 | build | approved | **Ledger tooling — one job, not four.** Merges items 10, 15, 19 and 22 (rulings 0012, 0014, 0015, 0018). All four edit `scripts/ledger-check.ts`; doing them separately means four passes over one file. **Starts when Step 4 closes** — full handoff in `## Next authorized` below. | 2026-09-17 |
| 1 | decision | approved | **0002, 0003, 0006 confirmed** against code and migrations. **0005 and 0007 unconfirmed** — process decisions with no code artifact, and `search_session_transcripts` is unavailable in unsupervised mode. Retry with tool approval. Confirmations move into `.confirmed.json` once item 10 lands. | 2026-09-16 |
