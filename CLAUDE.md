...
- **Capture, don't retype.** Where the system already holds a fact, a model must select from it rather than write its own copy. Nearly every defect found in Build 1 has been one of two shapes: a model typing something the code already knew, or an interface asserting something the code had already decided otherwise. The fixes are correspondingly of two shapes — capture the real value and have the model reference it by index, or derive the displayed value from the rule instead of restating it. This is why the Research Agent selects `evidence_ids` instead of writing quotes, why a candidate's display name is derived from its parts, and why prompt wording is never the last line of defence: measured prompt-only compliance on this codebase is 52–80%, so any guarantee that matters belongs in code.
- **A field a model wrote is a claim, not a fact.** Store it with its provenance and whether the captured source supports it (see *capture contract* and *attestation*), and let each consumer decide what to do with an unsupported one. Never silently delete — "not evaluated" and "evaluated and clean" are different facts and must not collapse into one value.
...
- **Capture contract** — the rule that Donor Finder keeps the URLs and titles the web search actually returned, rather than letting the model recall them afterwards. A candidate that cannot be attributed to a real search result is marked `source_missing`: kept for audit, hidden from the queue, and never allowed to become a prospect. Provenance (`source_domain` — where it was found) and officialness (`website_status` — whose site this is) are recorded separately and never conflated; a directory page about a funder is not the funder speaking about itself.
- **Attestation** — whether a model-typed field is supported by the captured source text, tested by requiring every distinctive token to appear in the title and URL. Deliberately token containment rather than fuzzy similarity: a threshold loose enough to tolerate real wording variance also accepts an invented phrase that shares a word. Failures are recorded in `asserted_fields`, not deleted. Donor Finder's equivalent of the Research Agent's evidence ledger, scaled to the thinner text this pipeline actually captures.
